...
export function mockDashboard() {
    return {
        ...
        stats: [
            { label: "Lô đang theo dõi", value: String(mockBatches.length) },
            { label: "Bằng chứng đang chờ", value: String(mockBatches.reduce((sum, batch) => sum + batch.timeline.filter(event => event.proofStatus === "pending").length, 0)) },
            { label: "Chuyến vận chuyển mở", value: String(mockBatches.filter(batch => batch.currentState === "IN_TRANSPORT").length) }
        ]
    };
}
export async function mockSubmitCommand(batchId, command) {
    await delay(250);
    return {
        ok: true,
        batchId,
...
export async function mockSendSensorReading(request) {
    await delay(250);
