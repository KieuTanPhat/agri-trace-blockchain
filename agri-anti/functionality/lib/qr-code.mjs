...
export function createQrSvgPath(matrix) {
    const commands = [];
...
function setModule(matrix, reserved, x, y, dark, isReserved = true) {
...
function addFinder(matrix, reserved, startX, startY) {
    for (let y = -1; y <= 7; y += 1) {
        for (let x = -1; x <= 7; x += 1) {
            const xx = startX + x;
            const yy = startY + y;
...
function addFormatInfo(matrix, reserved) {
    const bits = calculateFormatBits();
    for (let index = 0; index <= 5; index += 1)
        setModule(matrix, reserved, 8, index, bit(bits, index));
    ...
    for (let index = 9; index < 15; index += 1)
        setModule(matrix, reserved, 14 - index, 8, bit(bits, index));
    for (let index = 0; index < 8; index += 1)
        setModule(matrix, reserved, SIZE - 1 - index, 8, bit(bits, index));
    for (let index = 8; index < 15; index += 1)
        setModule(matrix, reserved, 8, SIZE - 15 + index, bit(bits, index));
}
